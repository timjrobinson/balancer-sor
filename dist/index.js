...
var require$$0$3 = require('bignumber.js');
...
new require$$0$3.BigNumber(10).pow(18);
function scale(input, decimalPlaces) {
    const scalePow = new require$$0$3.BigNumber(decimalPlaces.toString());
    const scaleMul = new require$$0$3.BigNumber(10).pow(scalePow);
    return input.times(scaleMul);
}
function bnum(val) {
    return new require$$0$3.BigNumber(val.toString());
}
...
const bignumber_js_1 = require$$0__default$1['default'];
...
class WeightedPool {
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        if (swapType === exports.SwapTypes.SwapExactIn) {
            return bnum(formatFixed(poolPairData.balanceIn.mul(this.MAX_IN_RATIO).div(constants.WeiPerEther), poolPairData.decimalsIn));
        }
        else {
            return bnum(formatFixed(poolPairData.balanceOut.mul(this.MAX_OUT_RATIO).div(constants.WeiPerEther), poolPairData.decimalsOut));
        }
    }
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                const amt = WeightedMath_1._calcOutGivenIn(bnum(poolPairData.balanceIn.toString()), bnum(poolPairData.weightIn.toString()), bnum(poolPairData.balanceOut.toString()), bnum(poolPairData.weightOut.toString()), scale(amount, poolPairData.decimalsIn), bnum(poolPairData.swapFee.toString()));
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                // poolPair balances are normalised so must be scaled before use
                const amt = WeightedMath_1._calcInGivenOut(bnum(poolPairData.balanceIn.toString()), bnum(poolPairData.weightIn.toString()), bnum(poolPairData.balanceOut.toString()), bnum(poolPairData.weightOut.toString()), scale(amount, poolPairData.decimalsOut), bnum(poolPairData.swapFee.toString()));
                // return normalised amount
...
balances // balances
    ...
    let prevInv = ZERO;
    let inv = sum;
    const ampTimesNpowN = bnum(amp.toString()).times(Math.pow(totalCoins, totalCoins)); // A*n^n
    for (let i = 0; i < 255; i++) {
...
function _poolDerivatives$1(amp, balances, tokenIndexIn, tokenIndexOut, is_first_derivative, wrt_out) {
    ...
    const a = bnum(amp.toString()).times(Math.pow(totalCoins, totalCoins)); // = ampTimesNpowN
...
function _spotPriceAfterSwapTokenInForExactBPTOut(amount, poolPairData) {
    ...
    let ans = _poolDerivativesBPT(amp, balances, bnum(formatFixed(balanceOut, decimalsOut)).plus(amount), tokenIndexIn, true, true, true);
...
class StablePool {
    constructor(id, address, amp, swapFee, totalShares, tokens, tokensList) {
        ...
        this.AMP_PRECISION = BigNumber.from('1000');
        this.MAX_IN_RATIO = parseFixed('0.3', 18);
        this.MAX_OUT_RATIO = parseFixed('0.3', 18);
        this.id = id;
        this.address = address;
    ...
    getNormalizedLiquidity(poolPairData) {
        ...
        return bnum(formatFixed(poolPairData.balanceOut.mul(poolPairData.amp), poolPairData.decimalsOut));
    }
    getLimitAmountSwap(poolPairData, swapType) {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amt = StableMath_1._calcOutGivenIn(bnum(this.ampAdjusted.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amtScaled, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_DOWN mode (1)
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = StableMath_1._calcInGivenOut(bnum(this.ampAdjusted.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amtScaled, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_UP mode (0)
...
class MetaStablePool {
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        const allBalancesScaled = this.tokens.map(({ balance, priceRate }) => parseFixed(balance, 18).mul(parseFixed(priceRate, 18)).div(constants.WeiPerEther));
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amt = StableMath_1._calcOutGivenIn(bnum(this.ampAdjusted.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_DOWN mode (1)
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amt = StableMath_1._calcInGivenOut(bnum(this.ampAdjusted.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_UP mode (0)
    ...
    _spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        ...
        const result = _spotPriceAfterSwapExactTokenInForTokenOut$1(amountConverted, poolPairData);
        return result;
    }
    _spotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        const amountConverted = amount.times(formatFixed(poolPairData.tokenOutPriceRate, 18));
        const result = _spotPriceAfterSwapTokenInForExactTokenOut$1(amountConverted, poolPairData);
        return result;
    }
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
...
class ElementPool {
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        ...
        if (swapType === exports.SwapTypes.SwapExactIn) {
            ...
            return bnum(formatFixed(poolPairData.balanceOut.mul(MAX_OUT_RATIO).div(constants.WeiPerEther), poolPairData.decimalsOut));
        }
    }
    // Updates the balance of a given token for the pool
    updateTokenBalanceForPool(token, newBalance) {
...
const PRICE_ERROR_TOLERANCE = new require$$0$3.BigNumber(priceErrorTolerance);
...
const INFINITESIMAL = new require$$0$3.BigNumber(infinitesimal);
...
const optimizeSwapAmounts = (paths, swapType, totalSwapAmount, initialSwapAmounts, highestLimitAmounts, inputDecimals, outputDecimals, initialNumPaths, maxPools, costReturnToken) => {
    ...
    for (let b = initialNumPaths; b <= paths.length; b++) {
        if (b != initialNumPaths) {
            ...
            const newSwapAmount = require$$0$3.BigNumber.min.apply(null, [
        ...
        let selectedPaths = [];
        let [newSelectedPaths, exceedingAmounts, pathIds] = getBestPathIds(paths, swapType, swapAmounts, inputDecimals);
        ...
        while (!historyOfSortedPathIds.includes(sortedPathIdsJSON) && b > 1) {
            ...
            [swapAmounts, exceedingAmounts] = iterateSwapAmounts(selectedPaths, swapType, totalSwapAmount, swapAmounts, exceedingAmounts);
            [newSelectedPaths, exceedingAmounts, pathIds] = getBestPathIds(paths, swapType, swapAmounts, inputDecimals);
            if (pathIds.length === 0)
                break;
...
function iterateSwapAmounts(selectedPaths, swapType, totalSwapAmount, swapAmounts, exceedingAmounts) {
    ...
    while (priceError.isGreaterThan(PRICE_ERROR_TOLERANCE)) {
        ...
        const maxPrice = require$$0$3.BigNumber.max.apply(null, prices);
        const minPrice = require$$0$3.BigNumber.min.apply(null, prices);
...
iterationCount) {
    ...
    while (require$$0$3.BigNumber.min.apply(null, swapAmounts).lt(ZERO) ||
        require$$0$3.BigNumber.max.apply(null, exceedingAmounts).gt(ZERO)) {
...
const calcTotalReturn = (paths, swapType, swapAmounts, inputDecimals) => {
    let totalReturn = new require$$0$3.BigNumber(0);
...
const getBestPaths = (paths, swapType, totalSwapAmount, inputDecimals, outputDecimals, maxPools, costReturnToken) => {
    ...
    const [bestPaths, bestSwapAmounts, bestTotalReturnConsideringFees] = optimizeSwapAmounts(paths, swapType, bnum(formatFixed(totalSwapAmount, inputDecimals)), initialSwapAmounts.map((amount) => bnum(formatFixed(amount, inputDecimals))), highestLimitAmounts.map((amount) => bnum(formatFixed(amount, inputDecimals))), inputDecimals, outputDecimals, initialNumPaths, maxPools, costReturnToken);
...
function getPathsUsingStaBalPool(tokenIn, tokenOut, poolsAll, poolsFiltered, chainId) {
    ...
    if (metastablePoolIdIn && !metastablePoolIdOut) {
        ...
        const staBalPath = createMultihopPath(metaStablePoolIn, usdcConnectingPool, tokenIn, hopTokenStaBal, usdcConnectingPoolInfo.usdc);
        ...
        const pathEnd = createDirectPath(lastPool, usdcConnectingPoolInfo.usdc, tokenOut);
    ...
    if (!metastablePoolIdIn && metastablePoolIdOut) {
        ...
        const staBalPath = createMultihopPath(usdcConnectingPool, metaStablePoolIn, usdcConnectingPoolInfo.usdc, hopTokenStaBal, tokenOut);
        const pathStart = createDirectPath(firstPool, tokenIn, usdcConnectingPoolInfo.usdc);
...
function getLimitAmountSwapForPath(path, swapType) {
    ...
    if (poolPairData.length == 1) {
        if (swapType === exports.SwapTypes.SwapExactIn) {
            return parseFixed(path.pools[0]
                .getLimitAmountSwap(poolPairData[0], swapType)
                .dp(poolPairData[0].decimalsIn)
                .toString(), poolPairData[0].decimalsIn);
        }
        else {
            return parseFixed(path.pools[0]
                .getLimitAmountSwap(poolPairData[0], swapType)
                .dp(poolPairData[0].decimalsOut)
                .toString(), poolPairData[0].decimalsOut);
        }
    }
    else if (poolPairData.length == 2) {
        if (swapType === exports.SwapTypes.SwapExactIn) {
            ...
            if (limitOutputAmountSwap1.gt(limitAmountSwap2))
                ...
                // this is necessary to avoid return NaN
                    ...
                    return parseFixed(getOutputAmountSwap(path.pools[0], path.poolPairData[0], exports.SwapTypes.SwapExactOut, limitAmountSwap2)
                        .dp(poolPairData[0].decimalsIn)
                        .toString(), poolPairData[0].decimalsIn);
            // This means first hop is limiting the path
            else
                return parseFixed(limitAmountSwap1.toString(), poolPairData[0].decimalsIn);
        }
        else {
            const limitAmountSwap1 = path.pools[0].getLimitAmountSwap(poolPairData[0], swapType);
            const limitAmountSwap2 = path.pools[1].getLimitAmountSwap(poolPairData[1], swapType);
            ...
            if (limitOutputAmountSwap2.gt(limitAmountSwap1))
                // This means first hop is limiting the path
                return parseFixed(getOutputAmountSwap(path.pools[1], path.poolPairData[1], exports.SwapTypes.SwapExactIn, limitAmountSwap1)
                    .dp(poolPairData[1].decimalsOut)
                    .toString(), poolPairData[1].decimalsOut);
            // This means second hop is limiting the path
            else
                return parseFixed(limitAmountSwap2.dp(poolPairData[1].decimalsOut).toString(), poolPairData[1].decimalsOut);
        }
    }
    else {
        throw new Error('Path with more than 2 swaps not supported');
...
class SwapCostCalculator {
     ...
     */
    convertGasCostToToken(tokenAddress, tokenDecimals, gasPriceWei, swapGas = BigNumber.from('35000')) {
        return __awaiter(this, void 0, void 0, function* () {
            if (gasPriceWei.isZero() || swapGas.isZero())
                return constants.Zero;
            const tokenPrice = yield this.getNativeAssetPriceInToken(tokenAddress);
            const tokenPriceWei = BigNumber.from(scale(bnum(tokenPrice), tokenDecimals).dp(0).toString());
    ...
    */
    ...
    getSwaps(tokenIn, tokenOut, swapType, swapAmount, swapOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const wrappedInfo = yield getWrappedInfo(this.provider, swapType, tokenIn, tokenOut, this.chainId, BigNumber.from(swapAmount));
            let swapInfo;
    ...
    getBestPaths(paths, swapAmount, swapType, tokenInDecimals, tokenOutDecimals, costOutputToken, maxPools) {
        ...
        return [
            swaps,
            parseFixed(total.dp(outputDecimals, require$$0$3.BigNumber.ROUND_FLOOR).toString(), outputDecimals),
            marketSp,
            parseFixed(totalConsideringFees
                .dp(outputDecimals, require$$0$3.BigNumber.ROUND_FLOOR)
...
function BPTForTokensZeroPriceImpact$1(balances, decimals, normalizedWeights, amounts, bptTotalSupply) {
    let amountBPTOut = new require$$0$3.BigNumber(0);
    ...
    for (let i = 0; i < balances.length; i++) {
        ...
        amounts[i] = amounts[i].times(new require$$0$3.BigNumber(10).pow(-decimals[i]));
        const poolPairData = {
    ...
    return amountBPTOut.times(new require$$0$3.BigNumber(10).pow(18));
}
...
bptTotalSupply, amp) {
    ...
    for (let i = 0; i < allBalances.length; i++) {
        allBalancesDownScaled.push(allBalances[i].times(new require$$0$3.BigNumber(10).pow(-decimals[i])));
    }
    for (let i = 0; i < allBalances.length; i++) {
