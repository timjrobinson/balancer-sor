import require$$0$3, { BigNumber as BigNumber$1 } from 'bignumber.js';
...
import { WeiPerEther, MaxUint256, Zero as Zero$1, AddressZero } from '@ethersproject/constants';
...
new BigNumber$1(10).pow(18);
function scale(input, decimalPlaces) {
    const scalePow = new BigNumber$1(decimalPlaces.toString());
    const scaleMul = new BigNumber$1(10).pow(scalePow);
    return input.times(scaleMul);
}
function bnum(val) {
    return new BigNumber$1(val.toString());
}
...
const bignumber_js_1 = require$$0$3;
...
const PRICE_ERROR_TOLERANCE = new BigNumber$1(priceErrorTolerance);
...
const INFINITESIMAL = new BigNumber$1(infinitesimal);
...
const optimizeSwapAmounts = (paths, swapType, totalSwapAmount, initialSwapAmounts, highestLimitAmounts, inputDecimals, outputDecimals, initialNumPaths, maxPools, costReturnToken) => {
    ...
    for (let b = initialNumPaths; b <= paths.length; b++) {
        if (b != initialNumPaths) {
            ...
            const newSwapAmount = BigNumber$1.min.apply(null, [
...
function iterateSwapAmounts(selectedPaths, swapType, totalSwapAmount, swapAmounts, exceedingAmounts) {
    ...
    while (priceError.isGreaterThan(PRICE_ERROR_TOLERANCE)) {
        ...
        const maxPrice = BigNumber$1.max.apply(null, prices);
        const minPrice = BigNumber$1.min.apply(null, prices);
...
iterationCount) {
    ...
    while (BigNumber$1.min.apply(null, swapAmounts).lt(ZERO) ||
        BigNumber$1.max.apply(null, exceedingAmounts).gt(ZERO)) {
...
const calcTotalReturn = (paths, swapType, swapAmounts, inputDecimals) => {
    let totalReturn = new BigNumber$1(0);
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            ...
            if (limitOutputAmountSwap1.gt(limitAmountSwap2))
                ...
                // this is necessary to avoid return NaN
                    ...
                    return parseFixed(getOutputAmountSwap(path.pools[0], path.poolPairData[0], SwapTypes.SwapExactOut, limitAmountSwap2)
                        .dp(poolPairData[0].decimalsIn)
                        .toString(), poolPairData[0].decimalsIn);
            ...
            if (limitOutputAmountSwap2.gt(limitAmountSwap1))
                // This means first hop is limiting the path
                return parseFixed(getOutputAmountSwap(path.pools[1], path.poolPairData[1], SwapTypes.SwapExactIn, limitAmountSwap1)
...
class SOR {
    ...
    */
    ...
    getBestPaths(paths, swapAmount, swapType, tokenInDecimals, tokenOutDecimals, costOutputToken, maxPools) {
        ...
        return [
            swaps,
            parseFixed(total.dp(outputDecimals, BigNumber$1.ROUND_FLOOR).toString(), outputDecimals),
            marketSp,
            parseFixed(totalConsideringFees
                .dp(outputDecimals, BigNumber$1.ROUND_FLOOR)
...
function BPTForTokensZeroPriceImpact$1(balances, decimals, normalizedWeights, amounts, bptTotalSupply) {
    let amountBPTOut = new BigNumber$1(0);
    ...
    for (let i = 0; i < balances.length; i++) {
        ...
        amounts[i] = amounts[i].times(new BigNumber$1(10).pow(-decimals[i]));
        const poolPairData = {
    ...
    return amountBPTOut.times(new BigNumber$1(10).pow(18));
}
...
bptTotalSupply, amp) {
    ...
    for (let i = 0; i < allBalances.length; i++) {
        allBalancesDownScaled.push(allBalances[i].times(new BigNumber$1(10).pow(-decimals[i])));
    }
    for (let i = 0; i < allBalances.length; i++) {
